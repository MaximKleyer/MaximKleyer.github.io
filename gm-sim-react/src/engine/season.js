...
export function completeCurrentInternational(gameState) {
  ...
  gameState.season.history.push({
    ...
    pointsAwarded,
    // Light summary for History tab (Phase 5)
...
export function beginNextSlot(gameState) {
  ...
  while (true) {
    ...
    // Worlds (and any other future types) still placeholder-skip in Phase 3.
    // Phase 4 will replace this branch with real Worlds handling.
    s.history.push({
      slotIndex: s.slotIndex,
      type: slot.type,
      name: slot.name,
      placeholder: true,
      note: 'Not yet implemented — coming in a later phase.',
      pointsAwarded: {},
    });
  }
}
 ...
 * Wipes records, resets player stats, reshuffles groups, regenerates schedule.
 * Free agents are left alone.
 */
